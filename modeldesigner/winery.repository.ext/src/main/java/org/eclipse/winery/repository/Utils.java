...
import org.eclipse.winery.repository.export.ZipExporter;
import org.eclipse.winery.repository.ext.export.custom.ExportFileGenerator;
import org.eclipse.winery.repository.ext.repository.RepositoryUploadService;
...
/**
 * Contains utility functionality concerning with everything that is <em>not</em> related only to
 * the repository, but more. For instance, resource functionality. Utility functionality for the
 * repository is contained at {@link BackendUtils}
 */
public class Utils {
  ...
  private static final String RANGE_NCNAMESTARTCHAR =
   ...
   * org.eclipse.winery.common.Util.makeNCName(String)}. The methods should be merged into one.
   * 
   */
   ...
   * @param uri
   */
  public static Response getDefinitionsOfSelectedResource(
      final AbstractComponentInstanceResource resource, final URI uri,
      final ExportFileGenerator extendPoint) {
    ...
    StreamingOutput so = new StreamingOutput() {
      ...
      public void write(OutputStream output) throws IOException, WebApplicationException {
        ...
        try {
          exporter.exportTOSCA(resource.getId(), output, conf, extendPoint);
    ...
    /*
     * this code is for offering a download action // Browser offers save as // .tosca is more or
     * less needed for debugging, only a CSAR makes sense. // Therefore, we want to have the xml
     * opened in the browser. StringBuilder sb = new StringBuilder();
     * sb.append("attachment;filename=\""); sb.append(resource.getXmlId().getEncoded());
     * sb.append(" - "); sb.append(resource.getNamespace().getEncoded()); sb.append(".xml");
     * sb.append("\""); return Response.ok().header("Content-Disposition", sb
  ...
  public static Response getCSARofSelectedResource(final AbstractComponentInstanceResource resource) {
    ...
    sb.append("\"");
    return Response.ok().header("Content-Disposition", sb.toString())
        .type(org.eclipse.winery.common.constants.MimeTypes.MIMETYPE_ZIP).entity(so).build();
  }


  public static Response getZipOfSelectedResource(final AbstractComponentInstanceResource resource) {
    ...
    StreamingOutput so = new StreamingOutput() {
      ...
      public void write(OutputStream output) throws IOException, WebApplicationException {
        try {
          exporter.writeZip(resource.getId(), output);
        } catch (Exception e) {
          throw new WebApplicationException(e);
        }
      }
  ...
  public static Response getCSARDeployResource(final AbstractComponentInstanceResource resource) {
    ...
    try {
      // write CSAR file
      String path = Utils.class.getResource("/").getPath() + getRandomString(6);

      // created a random dir
      File filePath = new File(path);
      filePath.mkdir();

      // add random string to CSAR name
      String fileName =
          path + "/" + resource.getXmlId().getEncoded()
              + org.eclipse.winery.repository.Constants.SUFFIX_CSAR;
      Utils.logger.trace("write filename=" + fileName);
      File csarFile = new File(fileName);

      FileOutputStream writeFile = new FileOutputStream(csarFile);
      // get csar outputstream
      OutputStream csar = new ByteArrayOutputStream();
      exporter.writeCSAR(resource.getId(), csar);
      Utils.logger.trace("begin writeCSAR!");
      writeFile.write(((ByteArrayOutputStream) csar).toByteArray());
      writeFile.flush();
      writeFile.close();
      Utils.logger.trace("begin to send and deploy CSAR!");

      // String serverUrl = "http://ip/api/nsoc/v1/csars";
      CSARDeployClient client = new CSARDeployClient("127.0.0.1", "80");
      ...
      logger.error("publish csar error", e);
    }
  ...
  public static Response getZipDeployResource(final AbstractComponentInstanceResource resource,
    ...
    try {
      ...
      writeFile = new FileOutputStream(csarFile);
      // get csar outputstream
      csar = new ByteArrayOutputStream();
      exporter.writeZip(resource.getId(), csar);
      Utils.logger.trace("begin writeCSAR!");
      writeFile.write(((ByteArrayOutputStream) csar).toByteArray());
      writeFile.flush();
      writeFile.close();
      ...
      Set<Class<? extends RepositoryUploadService>> impls = getRepositoryServiceImpl();
      Utils.logger.info("find" + impls.size() + " implementions to deploy csar file");
      Iterator it = impls.iterator();
      while (it.hasNext()) {
        Class<? extends RepositoryUploadService> implClass =
            (Class<? extends RepositoryUploadService>) it.next();
        Utils.logger.info("the" + implClass.toString() + "begin to publish files");
        try {
          ...
          logger.error("fail to instance " + implClass.toString(), e);
        } catch (IllegalAccessException e) {
          logger.error("publish zip error", e);
        }
      }
      Utils.logger.trace("send and deploy CSAR finish!");
      if (isSuccess)
        jsonRes = "[{\"result\":\"" + "OK" + "\"}]";
  ...
  private static String getPackageType(AbstractComponentInstanceResource resource) {
    String namespace = resource.getNamespace().getDecoded();
    if (namespace.endsWith(Constants.NFV_TYPE_NS)) {
      return Constants.NFV_TYPE_NS;
    } else if (namespace.endsWith(Constants.NFV_TYPE_VNF)) {
      return Constants.NFV_TYPE_VNF;
    }
    return null;
  }

  private static String getDesc(AbstractComponentInstanceResource resource) {
    String desc = null;
    if (resource instanceof ServiceTemplateResource) {
      ServiceTemplateResource stResource = (ServiceTemplateResource) resource;
      List<TDocumentation> documentation = stResource.getServiceTemplate().getDocumentation();
      if (null != documentation && !documentation.isEmpty()) {
        List<Object> content = documentation.get(0).getContent();
        if (null != content && !content.isEmpty()) {
          desc = content.get(0).toString();
        }
      }
    }
  ...
  public static Set<Class<? extends RepositoryUploadService>> getRepositoryServiceImpl() {
    Reflections reflections = new Reflections("org.eclipse.winery.repository.ext");
    return reflections
        .getSubTypesOf(org.eclipse.winery.repository.ext.repository.RepositoryUploadService.class);
  }

  /**
   * @return Singular type name for the given resource. E.g., "ServiceTemplateResource" gets
   *         "ServiceTemplate"
   */
  public static String getTypeForInstance(
      Class<? extends AbstractComponentInstanceResource> resClass) {
  ...
  public static String getTypeForComponentContainer(
  ...
  public static String getRelativeURL(URI baseURI, GenericId id) {
    String absolutePath =
        Prefs.INSTANCE.getResourcePath() + "/"
            + Utils.getURLforPathInsideRepo(BackendUtils.getPathInsideRepo(id));
  ...
  /**
   * This method is similar to {@link org.eclipse.winery.common.Util.qname2href()}, but treats
   * winery's internal ID model instead of the global TOSCA model
   * 
  ...
  public static String getHREF(TOSCAComponentId id) {
    String res =
        "<a href=\"" + Utils.getAbsoluteURL(id) + "\">"
            + Functions.escapeXml(id.getXmlId().getDecoded()) + "</a>";
    return res;
  }
  ...
  public static Collection<NodeTypeResource> getAllNodeTypeResources() {
    ...
    Collection<NodeTypeResource> res =
        (Collection<NodeTypeResource>) (Collection<?>) new NodeTypesResource().getAll();
    return res;
  }
  ...
  public static Collection<RelationshipTypeResource> getAllRelationshipTypeResources() {
    ...
    Collection<RelationshipTypeResource> res =
        (Collection<RelationshipTypeResource>) (Collection<?>) new RelationshipTypesResource()
            .getAll();
    return res;
  }
   ...
   */
  public static MediaType getFixedMimeType(BufferedInputStream is, String fileName,
      MediaType mediaType) {
   ...
   * 
   * We cannot use {@literal Class<? extends TExtensibleElements>} as, for instance,
   * {@link TConstraint} does not inherit from {@link TExtensibleElements}
   * 
  ...
  public static <T extends Object> String getXMLAsString(Class<T> clazz, T obj,
      boolean includeProcessingInstruction) {
  ...
  public static String getAllXSDefinitionsForTypeAheadSelection(short type) {
    SortedSet<XSDImportId> allImports =
        Repository.INSTANCE.getAllTOSCAComponentIds(XSDImportId.class);
  ...
  public static TEntityType getTypeForTemplate(TEntityTemplate template) {
    ...
    // strip off "Template"
    instanceResourceClassName =
        instanceResourceClassName.substring(0,
    ...
    String idClassName =
        "org.eclipse.winery.common.ids.definitions." + instanceResourceClassName + "Id";

    String packageName =
        "org.eclipse.winery.repository.resources.entitytypes."
    ...
    try {
      instanceResourceClass =
          (Class<? extends AbstractComponentInstanceResource>) Class
              .forName(instanceResourceClassName);
    } catch (ClassNotFoundException e) {
   ...
   * referenced by functions.tld
   * 
   * We need the bridge as functions (at tld) require a static method. We did not want to put two
   * methods in Prefs and therefore, we put the method here.
   */
  ...
  /**
   * Converts the given String to an integer. Fallback if String is a float. If String is an invalid
  ...
  @SuppressWarnings("rawtypes")
  public static TDefinitions getDefiniton(String namespace, String id,
      Class<? extends AbstractComponentsResource> containerClass) {
    Class<? extends TOSCAComponentId> idClass =
        getComponentIdClassForComponentContainer(containerClass);
    ...
    AbstractComponentInstanceResource res =
        AbstractComponentsResource.getComponentInstaceResource(componetId);
    Definitions entryDefinitions = res.getDefinitions();
    return entryDefinitions;
  }

  public static TPolicy buildPolicyInfo(PolicyExtInfo info) {
    if (null == info) {
      return null;
    }
    ...
    policy.setPolicyType(info.getType());
    Map<QName, String> otherAttributes = policy.getOtherAttributes();
    putAtrribute(otherAttributes, "desc", info.getDesc());
    putAtrribute(otherAttributes, "target", new Gson().toJson(info.getTarget()));
    putAtrribute(otherAttributes, "derived_from", new Gson().toJson(info.getDerived_from()));
    putAtrribute(otherAttributes, "properties", new Gson().toJson(info.getProperties()));
    return policy;
  }

  @SuppressWarnings("unchecked")
  public static PolicyExtInfo buildTPolicy(TPolicy policy) {
    if (null == policy) {
      return null;
    }

    PolicyExtInfo info = new PolicyExtInfo();
    info.setName(policy.getName());
    info.setType(policy.getPolicyType());
    Map<QName, String> otherAttributes = policy.getOtherAttributes();
    info.setDesc(getAtrribute(otherAttributes, "desc"));
    info.setTarget((new Gson().fromJson(getAtrribute(otherAttributes, "target"), List.class)));
    info.setDerived_from(new Gson().fromJson(getAtrribute(otherAttributes, "derived_from"),
        QName.class));
    info.setProperties(new Gson().fromJson(getAtrribute(otherAttributes, "properties"), Map.class));
    return info;
  }

  private static void putAtrribute(Map<QName, String> otherAttributes, String key, String value) {
    if (null != key && null != value && !value.isEmpty()) {
      otherAttributes.put(new QName(key), value);
    }
  }

  private static String getAtrribute(Map<QName, String> otherAttributes, String key) {
    if (null != key) {
      return otherAttributes.get(new QName(key));
    }
    return null;
  }

  public static boolean isServiceTemplateDefinition(Definitions definitions) {
    List<TExtensibleElements> contents =
        definitions.getServiceTemplateOrNodeTypeOrNodeTypeImplementation();
    for (TExtensibleElements content : contents) {
      if (content instanceof TServiceTemplate)
        return true;
    }
    return false;
  }
