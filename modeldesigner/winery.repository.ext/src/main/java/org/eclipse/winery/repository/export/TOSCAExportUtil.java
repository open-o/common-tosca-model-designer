...
public class TOSCAExportUtil {
  ...
  private ArrayList<DefinitionResultInfo> yamlExportDefResultList =
      new ArrayList<DefinitionResultInfo>();
  ...
  public Collection<TOSCAComponentId> exportTOSCA(TOSCAComponentId id, OutputStream out,
      Map<String, Object> exportConfiguration, ExportFileGenerator extendPoint) throws IOException,
      JAXBException {
    ...
    if (res.getElement() instanceof TEntityType) {
      ...
      if (wpd != null) {
        if (wpd.getIsDerivedFromXSD() == null) {
          ...
          // Determine location
          String loc =
              BackendUtils.getImportLocationForWinerysPropertiesDefinitionXSD((EntityTypeId) tcId,
                  uri, wrapperElementLocalName);
          if (uri == null) {
            ...
            if (!isValueExist(this.referencesToPathInCSARMap, locInCSAR))
              this.referencesToPathInCSARMap.put(new DummyRepositoryFileReferenceForGeneratedXSD(
                  document), locInCSAR);
          }
          imp.setLocation(loc);
  ...
  private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(NodeTypeId id) {
    ...
    Collection<NodeTypeImplementationId> allNodeTypeImplementations =
        BackendUtils.getAllElementsRelatedWithATypeAttribute(NodeTypeImplementationId.class,
            id.getQName());
