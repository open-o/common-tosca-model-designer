...
import org.eclipse.winery.common.boundaryproperty.Outputs;
...
public class BoundaryPropertyDefinitionsResource {
    ...
    @RestDoc(
            resourceDescription = "Models the user-defined properties. The property mappings go into a separate resource propertymappings.")
    public Response putProperties(
            @RestDocParam(
                    description = "Stored properties. The XSD allows a single element only. Therefore, we go for the contained element") Document doc) {
    ...
    @Path("outputs")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response addOutput(PropertyDefinitionKV output) {
        BoundaryPropertyDefinition def =
                BoundaryPropertyUtil.getBoundaryPropertyDefinition(this.property.getAny());
        if (null == def) {
            return Response.notModified().build();
    ...
    @Path("outputs")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response delOutput(@QueryParam(value = "key") String outputKey) {
        BoundaryPropertyDefinition def =
                BoundaryPropertyUtil.getBoundaryPropertyDefinition(this.property.getAny());
        if (null == def) {
            return Response.status(Status.NOT_FOUND).build();
    ...
    public List<PropertyDefinitionKV> getOutputs() {
        ...
        return def.getOutputs().getOutputs();
    }

    @POST
    @Path("outputs/list")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response addOutputList(Outputs outputs) {
        BoundaryPropertyDefinition def =
                BoundaryPropertyUtil.getBoundaryPropertyDefinition(this.property.getAny());
        if (null == def) {
            return Response.notModified().build();
