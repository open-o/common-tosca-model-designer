...
public class PlanResource extends EntityWithIdResource<TPlan> implements IHasName {
	...
	public Response getHTML(@Context UriInfo uriInfo) {
		...
		if (isBPMN4TOSCA && (!StringUtils.isEmpty(bpmn4toscaBaseURL))) {
			...
			// the next line is added by lvbo 2015.12.29. this line changes ip:port/winery to ip:port/BPMN4TOSCAModeler
//			uri = uriInfo.getBaseUri().resolve(uri).toString();
			uri = repositoryURI.toString() + "BPMN4TOSCAModeler";
