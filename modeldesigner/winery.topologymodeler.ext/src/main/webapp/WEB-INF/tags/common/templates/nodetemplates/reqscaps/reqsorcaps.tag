	...
	<div class="content">
		<c:forEach var="item" items="${list}">
			<div class="reqorcap ${cssClassPrefix} row" id="${item.id}">
				...
				<c:forEach var="map" items="${item.otherAttributes}">
					<c:choose>
					    <c:when test="${map.key == 'node'}">
					       <div class="col-xs-4 node reqorcap">${map.value}</div>
					    </c:when>
					    <c:when test="${map.key == 'capability'}">
					       <div class="col-xs-4 capability reqorcap">${map.value}</div>
					    </c:when>
					</c:choose>
				</c:forEach>
...
<script type="text/x-tmpl" id="tmpl-${shortName}">
	<div class="reqorcap ${cssClassPrefix}" id="{%=o.id%}">
		...
		<c:forEach var="map" items="${o.otherAttributes}">
			<c:choose>
			    <c:when test="${map.key == 'node'}">
			       <div class="col-xs-4 node reqorcap">${map.value}</div>
			    </c:when>
