...
<%
	...
	String serviceTemplateType = request.getParameter("type");
	if (StringUtils.isEmpty(serviceTemplateType)) {
		serviceTemplateType = "";
	}
%>
<!DOCTYPE html>
...
<div id="winery">
	...
	<div id="topbar">
		<button class="zte-btn zte-white" id="goBack" data-loading-text="loading...">
			<span id="winery-toolbar-btn-return" name_i18n="winery_i18n"></span>
		</button>
		<button class="zte-btn zte-white zte-multi" onclick="winery.events.fire(winery.events.name.command.SAVE);" id="saveBtn" data-loading-text="Saving...">
			<span id="winery-toolbar-btn-save" name_i18n="winery_i18n"></span>
		</button>
		...
		$("#deployCSARbtn").on("click", function(evt)  {
			...
			require(["winery-topologymodeler-AMD"], function(wt) {
				dConfirmYesNo($.i18n.prop("winery-publish-message-confirm","<%=serviceTemplateName%>"), function(){
					//save and publish
					wt.saveServiceTemplate();
					$.get(url, function(data) {
						//console.log(data);
						if(data[0].result == "OK") {
							vShowSuccess($.i18n.prop("winery-publish-message-success","<%=serviceTemplateName%>"));   
						} else {
							vShowError($.i18n.prop("winery-publish-message-error","<%=serviceTemplateName%>"));
						}
						restDialog();
					},'json');
				}, $.i18n.prop("winery-publish-message-title"));
			});
			return false;
		});
		</script>
...
<script>
	...
	$(function() {
		...
		//add by wandong, hide tosca.relationships.nfv.ForwardsTo relationship
		var localName = "tosca.relationships.nfv.ForwardsTo";
		var namespace = "http://www.zte.com.cn/tosca/nfv/vnffg";
