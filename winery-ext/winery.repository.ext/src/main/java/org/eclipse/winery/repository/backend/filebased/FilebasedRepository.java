...
public class FilebasedRepository extends AbstractRepository
            ...
            if (SystemUtils.IS_OS_WINDOWS) {
                if (new File(Constants.GLOBAL_REPO_PATH_WINDOWS).exists()) {
                    ...
                    try {
                        ...
                        FilebasedRepository.logger
                                .error("Could not create repository directory", e);
                    }
    ...
    public void forceDelete(RepositoryFileReference ref) throws IOException {
        ...
        try {
            ...
            Path mimeTypeFile =
                    fileToDelete.getParent().resolve(ref.getFileName() + Constants.SUFFIX_MIMETYPE);
     ...
     */
    @Override
    public void putContentToFile(RepositoryFileReference ref, InputStream inputStream,
        ...
        try {
            ...
            FilebasedRepository.logger.debug(
                    "Guessing that stream with length 0 is to be written to a file", e);
                ...
                try (DirectoryStream<Path> idDS = Files.newDirectoryStream(nsP, onhdf)) {
                    for (Path idP : idDS) {
                        ...
                        try {
                            ...
                            FilebasedRepository.logger.debug(
                                    "Internal error at determining id constructor", e);
                        ...
                        try {
                            ...
                            FilebasedRepository.logger.debug(
                                    "Internal error at invocation of id constructor", e);
                            // abort everything, return invalid result
                            return res;
                        }
                        res.add(id);
        ...
        try (DirectoryStream<Path> ds = Files.newDirectoryStream(dir, new OnlyNonHiddenFiles())) {
            for (Path p : ds) {
                RepositoryFileReference ref =
                        new RepositoryFileReference(id, p.getFileName().toString());
                res.add(ref);
            }
        } catch (IOException e) {
            FilebasedRepository.logger.debug("Cannot close ds", e);
    ...
    public <T extends TOSCAElementId> SortedSet<T> getNestedIds(GenericId ref, Class<T> idClass) {
        ...
        try (DirectoryStream<Path> ds =
                Files.newDirectoryStream(dir, new OnlyNonHiddenDirectories())) {
            for (Path p : ds) {
    ...
    public Collection<Namespace> getUsedNamespaces() {
        ...
        Class[] toscaComponentIds =
                {ArtifactTemplateId.class, ArtifactTypeId.class, CapabilityTypeId.class,
                        NodeTypeId.class, NodeTypeImplementationId.class, PolicyTemplateId.class,
                        PolicyTypeId.class, RelationshipTypeId.class,
                        RelationshipTypeImplementationId.class, RequirementTypeId.class,
    ...
    public void doClear() {
        try {
            ...
            DirectoryStream<Path> ds =
                    Files.newDirectoryStream(this.repositoryRoot, noGitDirFilter);
            for (Path p : ds) {
