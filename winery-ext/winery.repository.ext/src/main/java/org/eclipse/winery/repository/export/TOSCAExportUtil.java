...
public class TOSCAExportUtil {
    ...
    private Collection<String> yamlDefinitionNames = new ArrayList<>();
     ...
     * @param addRelatedComponents true: all referenced components (artifactTemplates,
     *        artifactTypes, ...) are added, false: only the XML belonging to the id is exported. If
     *        XML types are generated by Winery (e.g., the properties XSD for node types), these XML
    ...
    public Collection<TOSCAComponentId> exportTOSCA(TOSCAComponentId id, OutputStream out,
            Map<String, Object> exportConfiguration,ExportFileGenerator extendPoint) throws IOException, JAXBException {
     ...
     * artifactMap is filled to enable the CSAR exporter to create necessary entries in TOSCA-Meta
     * and to add them to the CSAR itself
     * 
    ...
    protected Collection<TOSCAComponentId> exportTOSCA(TOSCAComponentId id, OutputStream out,
            Map<RepositoryFileReference, String> referencesToPathInCSARMap,
            Map<String, Object> exportConfiguration,ExportFileGenerator extendPoint) throws IOException, JAXBException {
     ...
     * @throws JAXBException
     */
    private void writeDefinitionsElement(Definitions entryDefinitions, OutputStream out)
            throws JAXBException {
     ...
     */
    private Collection<TOSCAComponentId> writeDefinitionsElement(TOSCAComponentId tcId,
            OutputStream out,ExportFileGenerator extendPoint) throws IOException, JAXBException {
        ...
        AbstractComponentInstanceResource res =
                AbstractComponentsResource.getComponentInstaceResource(tcId);
        ...
        URI uri =
                (URI) this.exportConfiguration.get(TOSCAExportUtil.ExportProperties.REPOSITORY_URI
                        .toString());
        if (uri != null) {
        ...
        Collection<TOSCAComponentId> referencedTOSCAComponentIds =
                this.getReferencedTOSCAComponentIds(tcId);
        ...
        if (res.getElement() instanceof TEntityType) {
            ...
            WinerysPropertiesDefinition wpd =
                    ModelUtilities.getWinerysPropertiesDefinition(entityTypeRes.getEntityType());
            if (wpd != null) {
                if (wpd.getIsDerivedFromXSD() == null) {
                    ...
                    otherAttributes.put(QNames.QNAME_WINERYS_PROPERTIES_DEFINITION_ATTRIBUTE,
                            "true");

                    // Determine location
                    String loc =
                            BackendUtils.getImportLocationForWinerysPropertiesDefinitionXSD(
                    ...
                    if (uri == null) {
                        ...
                        Document document =
                                ModelUtilities.getWinerysPropertiesDefinitionXSDAsDocument(wpd);
                        ...
                        this.referencesToPathInCSARMap.put(
                                new DummyRepositoryFileReferenceForGeneratedXSD(document),
                                locInCSAR);
                    }
                    imp.setLocation(loc);
                    ...
                    propertiesDefinition.setType(new QName(wrapperElementNamespace,
                            wrapperElementLocalName));
        ...
        if(extendPoint != null){
            String[] fileNames = extendPoint.makeFile(entryDefinitions, out);
            if(extendPoint instanceof YamlExportFileGenerator){
                if(isServiceTemplateDefinition(entryDefinitions)){
                    this.yamlEntryDefinitionsReference = fileNames[0];
                }
                for(String fileName : fileNames){
                    yamlDefinitionNames.add(fileName);
                }
            }
        }else
    ...
    public boolean isServiceTemplateDefinition(Definitions entryDefinitions){
        List<TExtensibleElements>  contents = entryDefinitions.getServiceTemplateOrNodeTypeOrNodeTypeImplementation();
    ...
    public Collection<String> getYamlFileNames(){
        return yamlDefinitionNames;
    }
    ...
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(EntityTypeId id) {
        return this
                .getReferencedTOSCAComponentIdOfParentForAnAbstractComponentsWithTypeReferenceResource(id);
    }

    /**
     * There is now equivalent id class for
     * AbstractComponentInstanceResourceWithNameDerivedFromAbstractFinal, therefore we take the
     * super type and hope that the caller knows what he does.
     */
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIdOfParentForAnAbstractComponentsWithTypeReferenceResource(
            TOSCAComponentId id) {
        AbstractComponentInstanceResourceWithNameDerivedFromAbstractFinal res =
                (AbstractComponentInstanceResourceWithNameDerivedFromAbstractFinal) AbstractComponentsResource
        ...
        if (StringUtils.isEmpty(derivedFrom)) {
            ...
            try {
                ...
                throw new IllegalStateException("Could get constructor to instantiate parent id",
                        e1);
            }
            try {
    ...
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(TOSCAComponentId id) {
        ...
        if (id instanceof ServiceTemplateId) {
            ...
            referencedTOSCAComponentIds =
                    this.getReferencedTOSCAComponentIds((NodeTypeImplementationId) id);
        } else if (id instanceof RelationshipTypeId) {
            referencedTOSCAComponentIds =
                    this.getReferencedTOSCAComponentIds((RelationshipTypeId) id);
        } else if (id instanceof RelationshipTypeImplementationId) {
            referencedTOSCAComponentIds =
                    this.getReferencedTOSCAComponentIds((RelationshipTypeImplementationId) id);
        } else if (id instanceof RequirementTypeId) {
            referencedTOSCAComponentIds =
                    this.getReferencedTOSCAComponentIds((RequirementTypeId) id);
        } else if (id instanceof CapabilityTypeId) {
            referencedTOSCAComponentIds =
                    this.getReferencedTOSCAComponentIds((CapabilityTypeId) id);
            ...
            referencedTOSCAComponentIds =
                    this.getReferencedTOSCAComponentIds((PolicyTemplateId) id);
        ...
        if (id instanceof EntityTypeId) {
            Collection<TOSCAComponentId> additionalRefs =
                    this.getReferencedTOSCAComponentIds((EntityTypeId) id);
    ...
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(NodeTypeImplementationId id) {
        ...
        TImplementationArtifacts implementationArtifacts =
                res.getNTI().getImplementationArtifacts();
    ...
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(
            RelationshipTypeImplementationId id) {
        // We have to use a HashSet to ensure that no duplicate ids are added
        ...
        // IAs
        for (TImplementationArtifact ia : res.getRTI().getImplementationArtifacts()
    ...
    private Collection<TOSCAComponentId> prepareForExport(ServiceTemplateId id) {
        ...
        for (PlanId planId : nestedPlans) {
            SortedSet<RepositoryFileReference> containedFiles =
        ...
        if (res.getServiceTemplate().getTopologyTemplate() != null) {
            for (TEntityTemplate entityTemplate : res.getServiceTemplate().getTopologyTemplate()
                ...
                if (entityTemplate instanceof TNodeTemplate) {
                    ...
                    org.eclipse.winery.model.tosca.TNodeTemplate.Policies policies =
                            n.getPolicies();
                    if (policies != null) {
    ...
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(RelationshipTypeId id) {
        ...
        Collection<RelationshipTypeImplementationId> allTypeImplementations =
                BackendUtils.getAllElementsRelatedWithATypeAttribute(
                        RelationshipTypeImplementationId.class, id.getQName());
    ...
    private Collection<TOSCAComponentId> getReferencedTOSCAComponentIds(NodeTypeId id) {
        Collection<TOSCAComponentId> ids = new ArrayList<>();
        Collection<NodeTypeImplementationId> allNodeTypeImplementations =
                BackendUtils.getAllElementsRelatedWithATypeAttribute(
                        NodeTypeImplementationId.class, id.getQName());
    ...
    private void addVisualAppearanceToCSAR(TopologyGraphElementEntityTypeId id) {
        ...
        if (Repository.INSTANCE.exists(visId)) {
            ...
            RepositoryFileReference ref =
                    new RepositoryFileReference(visId, Filename.FILENAME_BIG_ICON);
