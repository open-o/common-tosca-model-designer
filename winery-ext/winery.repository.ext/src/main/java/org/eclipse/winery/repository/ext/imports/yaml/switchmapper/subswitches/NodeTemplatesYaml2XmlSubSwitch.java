 ...
 */
public class NodeTemplatesYaml2XmlSubSwitch extends AbstractYaml2XmlSubSwitch {

	public NodeTemplatesYaml2XmlSubSwitch(Yaml2XmlSwitch parentSwitch) {
		super(parentSwitch);
	}
    ...
    /**
     * Processes all YAML node templates by creating a
     * {@link org.opentosca.model.tosca.TNodeTemplate} and adding it to
     * {@link #getTopologyTemplate()}. In the next step, the relationship
     * requirements are processed. This depends on the creation of the node
     * templates before, because relationships between node templates can only
     * be established when all node templates are processed completely, because
     * the node template object is needed.
     */
    @Override
    public void process() {
        if (getServiceTemplate().getTopology_template().getNode_templates() != null) {
            for (final Entry<String, NodeTemplate> yamlNodeTemplate : getServiceTemplate()
                    .getTopology_template().getNode_templates().entrySet()) {
                final TNodeTemplate nodeTemplate = createNodeTemplate(
                        yamlNodeTemplate.getValue(), yamlNodeTemplate.getKey());
                getTopologyTemplate().getNodeTemplateOrRelationshipTemplate()
                        .add(nodeTemplate);
            }
            for (final Entry<TNodeTemplate, List<Map<String, Object>>> relationshipRequirement : this.relationshipRequirements
                    .entrySet()) {
                for (final Map<String, Object> requirement : relationshipRequirement
                        .getValue()) {
                    processRelationshipRequirements(
                            relationshipRequirement.getKey(), requirement);
                }
            }
        }
    }
    ...
    /**
     * Creates a node template object and processes the inner attributes like
     * capabilities, properties and requirements.
     * 
     * @param nodeTemplate
     *            YAML node template
     ...
     */
    private TNodeTemplate createNodeTemplate(NodeTemplate nodeTemplate,
        ...
        if (nodeTemplate.getDescription() != null
                && !nodeTemplate.getDescription().isEmpty()) {
            result.getDocumentation().add(
                    toDocumentation(nodeTemplate.getDescription()));
        }
        result.setType(getTypeMapperUtil().getCorrectTypeReferenceAsQName(
                nodeTemplate.getType(), ElementType.NODE_TYPE));
        ...
        if (nodeTemplate.getRequirements() != null
                && !nodeTemplate.getRequirements().isEmpty()) {
     ...
     * 
     * @param nodeTemplate
     *            YAML node template containing capabilities
     * @param result
     *            Tosca node template
     */
    private void processCapabilitiesInNodeTemplate(NodeTemplate nodeTemplate,
            TNodeTemplate result) {
        final TNodeTemplate.Capabilities capabilities = new TNodeTemplate.Capabilities();

        for (final Entry<String, Capability> nodeTemplateCapability : nodeTemplate
                .getCapabilities().entrySet()) {
            // if (nodeTemplateCapability.getValue() instanceof HashMap) {
            // final Map<?, ?> capabilityDefinition = (Map<?, ?>)
            ...
            // try {
            // capabilityType = nodeTemplateCapability.getValue()
            // .getType();
            // } catch (final Exception e) {
            // capabilityType = "CAPABILITY_TYPE";
            // }
            // tCapability.setType(getTypeMapperUtil()
            // .getCorrectTypeReferenceAsQName(capabilityType,
            // ElementType.CAPABILITY_TYPE));
                tCapability.setId(result.getId() + "_"
                        + nodeTemplateCapability.getKey());
    ...
    /**
     * Processes the node template properties by using
     * {@link #getAnyMapForProperties(java.util.Map, String)}.
     * 
     * @param nodeTemplate
     *            YAML node template containing properties
     * @param nodeTemplateName
     *            name of node template
     * @param result
     *            Tosca node template where the properties have to be stored in
     */
    private void processPropertiesInNodeTemplate(NodeTemplate nodeTemplate,
            String nodeTemplateName, TNodeTemplate result) {
        final TEntityTemplate.Properties prop = new TEntityTemplate.Properties();
        final JAXBElement<AnyMap> jaxbprop = getAnyMapForProperties(
    ...
    /**
     * Process requirements from {@code nodeTemplate} and add them to
     * {@code result}. There are two possible notations of requirements: <br />
     ...
     * </pre>
     * 
     * NOTE: There is a convention for "someCapability": It must end with
     * "Capability", otherwise no requirement will be created! <br />
     * 2) requirements: - relationshipTemplateId: targetElementId
     * relationship_type: someRelationshipType => will produce:<br>
     * 
     * <pre>
     ...
     * 
     * @param nodeTemplate
     *            node template from YAML
     * @param result
     *            corresponding node template for XML
     * @throws RuntimeException
     *             if the requirement does not match "^(\w+)Capability$"
     */
    private void processRequirements(final NodeTemplate nodeTemplate,
            final TNodeTemplate result) {
        final TNodeTemplate.Requirements resultRequirements = new TNodeTemplate.Requirements();
        this.relationshipRequirements.put(result,
                new ArrayList<Map<String, Object>>());

        // process all requirements
        for (final Map<String, Object> requirement : nodeTemplate
                .getRequirements()) {
     ...
     * 
     * @param result
     *            node template object for XML
     * @param resultRequirements
     *            object containing requirements for {@code result}
     * @param requirement
     */
    private void processSingleRequirement(final TNodeTemplate result,
            final TNodeTemplate.Requirements resultRequirements,
            final Map<String, Object> requirement) {
        if (requirement.containsKey("relationship_type")
                && requirement.size() == 2) {
            // Here: produce relationship template based on requirement
            // store requirement for later processing, because we need to access
            // the processed node templates
            // -> otherwise it's possible to request a node template which
            // hasn't processed so far
            ...
            throw new RuntimeException(
                    "This type of requirements definition is not supported.");
        }
    }
     ...
     * 
     * @param resultRequirements
     *            object storing requirements for a corresponding XML node
     *            template
     * @param requirement
     *            map containing requirement attributes; mapping size must be 1
     */
    private void createRequirement(
        ...
        if (capability.endsWith("Capability")) {
            final String requirementName = (String) requirement.keySet()
                    .toArray()[0];
            final String requirementTypeName = capability.replace("Capability",
                    "Requirement");
            ...
            tRequirement.setType(getTypeMapperUtil()
     ...
     * @param requirement
     */
    private void processRelationshipRequirements(final TNodeTemplate result,
            final Map<String, Object> requirement) {
        ...
        for (final String key : requirement.keySet()) {
            if (key.equals("relationship_type")) {
                ...
                relationshipTemplate.setType(getTypeMapperUtil()
                        .getCorrectTypeReferenceAsQName(relationshipType,
                ...
                final TNodeTemplate targetTemplate = getTargetNodeTemplate((String) requirement
                        .get(key));
                if (targetTemplate == null) {
                    throw new RuntimeException("Illegal reference. "
                            + (String) requirement.get(key)
                            + " is no valid NodeTemplate id.");
                }
        ...
        getTopologyTemplate().getNodeTemplateOrRelationshipTemplate().add(
                relationshipTemplate);
    }

    /**
     * Finds the {@link org.opentosca.model.tosca.TNodeTemplate} with id
     * {@code nodeTemplateId} by searching all available entity templates from
     * {@link #getTopologyTemplate()} and compare the id's.
     * 
     * @param nodeTemplateId
     *            id of the node template to search for
     * @return the node template object or null if node template couldn't be
     *         found
     */
    private TNodeTemplate getTargetNodeTemplate(final String nodeTemplateId) {
        for (final TEntityTemplate entityTemplate : getTopologyTemplate()
