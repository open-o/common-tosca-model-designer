...
public abstract class AbstractComponentInstanceResource implements Comparable<AbstractComponentInstanceResource>, IPersistable {
	 ...
	 */
	@DELETE
	public final Response onDelete(@QueryParam(value = "delSubstitutableNodeType") boolean delSubstitutableNodeType) {
		Response res = BackendUtils.delete(this.id);
		
		//if delete service template , delete the SubstitutableNodeType
		try {
		    TExtensibleElements element = this.getElement();
	        if(element instanceof TServiceTemplate){
	            QName subNode = ((TServiceTemplate) element).getSubstitutableNodeType();
	            if(subNode != null && delSubstitutableNodeType){
	                NodeTypesResource nres = new NodeTypesResource();
	                NodeTypeResource nodetypeRes =
	                         nres.getComponentInstaceResource(Util.URLencode(subNode.getNamespaceURI()), subNode.getLocalPart());
	...
	public final Response getHTML(@QueryParam(value = "definitions") String definitions, @QueryParam(value = "csar") String csar, @QueryParam(value = "exportType") String exportType,@Context UriInfo uriInfo) {
	...
	public final Response getCSARDeploy(@QueryParam(value = "deploy") String deploy,@QueryParam(value = "deployType") String deployType) {
		if (!Repository.INSTANCE.exists(this.id)) {
			return Response.status(Status.NOT_FOUND).build();
		}
		else if(deployType != null){
		    return Utils.getZipDeployResource(this);
		}
		else if(deploy != null)
		{
