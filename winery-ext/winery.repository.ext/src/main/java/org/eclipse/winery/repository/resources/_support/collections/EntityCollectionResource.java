...
/**
 * Class managing a list of entities. It is intended to manage subresources, which are stored in a
 * list. Either all entities have a unique key given by the TOSCA specification (subclass
 * EntityWithIdCollectionResource) or a unique key is generated (subclass
 * EntityWithoutIdCollectionResource)
 * 
 * @param <EntityResourceT> the resource modeling the entity
 * @param <EntityT> the entity type of single items in the list
 */
public abstract class EntityCollectionResource<EntityResourceT extends EntityResource<EntityT>, EntityT>
        implements
            IIdDetermination<EntityT> {
    ...
    /**
     * @param clazz the class of EntityT. Required as it is not possible to call new EntityT (see
     *        http://stackoverflow.com/a/1090488/873282)
     * @param list the list of entities contained in this resource. Has to be typed <Object> as not
     *        all TOSCA elements in the specification inherit from TExtensibleElements
     * @param res the main resource the list is belonging to. Required for persistence.
     */
    public EntityCollectionResource(Class<EntityResourceT> entityResourceTClazz, Class<EntityT> entityTClazz,
            List<EntityT> list, IPersistable res) {
    ...
    /**
     * Checks for containment of e in the list. <code>equals</code> is not used as most EntityT do
     * not offer a valid implementation
     * 
    ...
    @Path("entities")
    @Consumes({MediaType.TEXT_XML, MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public Response addNewElementList(List<EntityT> entities) {
        if (entities == null) {
            return Response.status(Status.BAD_REQUEST).entity("a valid XML/JSON element has to be posted").build();
        }
    ...
    protected void addEntity(EntityT entity) {
