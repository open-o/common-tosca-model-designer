...
<t:genericpage windowtitle="${windowtitle}" selected="${selected}" cssClass="mainContentContainer ${cssClass}" libs="${libs}">
	...
	<div class="top" style="display: none;">
		<c:if test="${not empty image}">
			<img src='visualappearance/50x50' style='position: absolute; margin-top: 32px; margin-left: 27px; height: 40px; width: 40px;display: none;' />
		</c:if>
		...
		<c:if test="${empty exception}">
			...
			<div style="float:right; margin-right:29px; margin-top: -10px; position:relative;display: none">
				<div style="float:right;">
					<button type="button" class="btn btn-danger mybtn" onclick="deleteResource('${it.name}', '.', openOverviewPage)">Delete</button>
					<a href="?definitions" class="btn btn-info mybtn">XML</a>
					<a href="?csar" class="btn btn-info mybtn">CSAR</a>
	...
	<div>
        <div class="subMenu" style="display:none;text-align: right;padding-top: 10px;margin-right: 10px;">
		    <%--
		    <button class="btn btn-primary" onclick="openOverviewPage()" id="goBack" data-loading-text="loading...">返回</button>
		    --%>
		...
		<div class="middle" id="ccontainer" style="margin-top: 5px;" >
		</div>
	</div>
