		...
		require(["winery-support-common", "winery-datatable"], function(wsc, wd) {
			...
			for(var i=0; i<propertiesTrs.length; i++) {
				...
				html += '<div class="form-group"><label class="col-sm-4 control-label">' + 
					name + '</label>' + '<div class="col-sm-8 col-fixed"><input name="' + name + '" value="'+ value + '" class="form-control" '+ disabledAttr + ' style="width:80%;float:left;"/>';
			...
			if(inputHtml) {
				inputHtml = '<div class="form-title"><span>输入参数</span></div>' + inputHtml;
			}
			if(metaDataHtml) {
				metaDataHtml = '<div class="form-title"><span>元数据</span></div>' + metaDataHtml;
			}
			propertyInfo.html(inputHtml + metaDataHtml);
