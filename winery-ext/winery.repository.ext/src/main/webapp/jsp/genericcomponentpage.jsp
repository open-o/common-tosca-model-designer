...
<div class="middle" id="ccontainer">
 	<div class="zte-btn-group">
		...
		<button type="button" class="zte-btn zte-primary" onclick="importCSAR();" style="margin-right: 10px;">导入</button>
	</div>
	<table class="zte-table" style="margin-left:20px;,margin-right:20px;width:96%">
		<thead>
			<tr>
				<th>名称</th>
				<th>命名空间</th>
		...
		<tr class="entityContainer ${it.CSSclass} ${loopStatus.index % 2 != 0 ? 'even' : 'myodd'}" id="${v:URLencode(t.namespace.encoded)}/${v:URLencode(t.xmlId.encoded)}/">
			...
			<td class="center">
				...
				<a href="${v:URLencode(t.namespace.encoded)}/${v:URLencode(t.xmlId.encoded)}/?csar&exportType" class="zte-btn zte-small">导出</a>
				<%--
				<a href="${v:URLencode(t.namespace.encoded)}/${v:URLencode(t.xmlId.encoded)}/?csar" class="zte-btn zte-small">导出</a>--%>
				<a href="${v:URLencode(t.namespace.encoded)}/${v:URLencode(t.xmlId.encoded)}/?edit" class="zte-btn zte-small editBtn">编辑</a>
				<%-- we need double encoding of the URL as the passing to javascript: decodes the given string once --%>
				<a href="javascript:deleteCI('${wc:escapeHtml4(t.xmlId.decoded)}', '${wc:escapeHtml4(t.namespace.decoded)}', '${v:URLencode(v:URLencode(t.namespace.encoded))}/${v:URLencode(v:URLencode(t.xmlId.encoded))}/');" class="zte-btn zte-small" onclick="element = $(this).parent().parent();">删除</a>
				<div class="right"></div>
			</td>
		</tr>
		</c:forEach>
